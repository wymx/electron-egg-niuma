...
const refreshRoomList = async () => {
  try {
    loading.value = true;
    // 先获取本地房间
    const localRooms = await ipcRenderer.invoke('get-room-list') || [];
    // 再发现局域网房间
    const lanRooms = await ipcRenderer.invoke('discover-rooms') || [];
    
    // 合并去重
    roomList.value = [...localRooms, ...lanRooms].reduce((acc, room) => {
      if (!acc.some(r => r.code === room.code)) {
        acc.push(room);
      }
      return acc;
    }, []);
